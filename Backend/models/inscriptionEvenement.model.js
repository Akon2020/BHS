...
const InscriptionEvenement = db.define(
  "InscriptionEvenement",
  {
    ...
    typeInscription: {
      ...
      allowNull: false,
      comment:
        "Indique si l'inscription provient d'un utilisateur connecté ou d'un visiteur",
    },
    dateInscription: {
      type: DataTypes.DATE,
  ...
  },
  {
    ...
    hooks: {
      beforeCreate: (inscription) => {
        inscription.typeInscription = inscription.idUtilisateur
          ? "utilisateur"
          : "visiteur";
      },
    ...
    validate: {
      validateInscription() {
        if (!this.nomComplet || !this.email || !this.sexe || !this.telephone) {
          throw new Error(
            "Le nom complet, l'email, le sexe et le téléphone sont obligatoires pour toute inscription",
          );
        }
      },
    },
